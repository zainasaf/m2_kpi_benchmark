...
## create single trust summary data frame ##
# Look for objects that end with "_summary" (our trust summary dataframes)
# Function to combine all trust summary dataframes into a single dataframe
combine_all_trust_summaries <- function() {
  # Get all objects in global environment
  all_objects <- ls(envir = .GlobalEnv)
  ...
  # Look for objects that end with "_summary" (our trust summary dataframes)
  summary_objects <- grep("_summary$", all_objects, value = TRUE)
  
  # Loop through each summary dataframe and append to all_summaries
  for (obj_name in summary_objects) {
    # Get the dataframe
    df <- get(obj_name, envir = .GlobalEnv)
    
    # Add the dataframe name as a new column
    df$Summary_Name <- obj_name
...
all_trust_summaries <- combine_all_trust_summaries()

# Print information about the combined dataframe
message(paste("Created combined dataframe with", nrow(all_trust_summaries), "rows"))
message(paste("Number of unique summaries:", length(unique(all_trust_summaries$Summary_Name))))

# Optional: Save to CSV
# write.csv(all_trust_summaries, "all_trust_summaries.csv", row.names = FALSE)

# Assign to global environment
assign("all_trust_summaries", all_trust_summaries, envir = .GlobalEnv)
...
save(list = ls(pattern = "_summary$"), file = "summary_data/top_3_index/kpi_sharing_99_ci.RData")
