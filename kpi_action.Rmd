...
add_action_to_all_dfs <- function() {
  ...
  # Debug: Print number of matching dataframes
  ...
  # Debug: Print the first matching dataframe name
  if(length(df_names) > 0) {
    message(paste("First matching dataframe:", df_names[1]))
    
    # Check unique values in each column for the first dataframe
    df_test <- get(df_names[1])
    message("Unique values in dochangemindQ2.O1: ", paste(unique(df_test$dochangemindQ2.O1), collapse=", "))
    message("Unique values in dolearnQ2.O2: ", paste(unique(df_test$dolearnQ2.O2), collapse=", "))
    message("Unique values in dononeQ2.O5: ", paste(unique(df_test$dononeQ2.O5), collapse=", "))
    message("Unique values in doshareQ2.O3: ", paste(unique(df_test$doshareQ2.O3), collapse=", "))
    message("Unique values in dowriteQ2.O4: ", paste(unique(df_test$dowriteQ2.O4), collapse=", "))
  }
  ...
  for (df_name in df_names) {
    ...
    # Debug: Check how many rows in each dataframe
    message(paste("Processing", df_name, "with", nrow(df), "rows"))
    
    # Try a different approach with direct column access
    action1_values <- as.numeric(df$dochangemindQ2.O1)
    action2_values <- as.numeric(df$dolearnQ2.O2)
    action3_values <- as.numeric(df$dononeQ2.O5)
    action4_values <- as.numeric(df$doshareQ2.O3)
    action5_values <- as.numeric(df$dowriteQ2.O4)
    
    # Create individual logical vectors
    action1 <- action1_values %in% c(1, 2)
    action2 <- action2_values %in% c(1, 2)
    action3 <- action3_values %in% c(1, 2)
    action4 <- action4_values %in% c(1, 2)
    action5 <- action5_values %in% c(1, 2)
    
    # Debug: Check how many TRUE values in each action
    message(paste("action1 TRUE count:", sum(action1, na.rm=TRUE)))
    message(paste("action2 TRUE count:", sum(action2, na.rm=TRUE)))
    message(paste("action3 TRUE count:", sum(action3, na.rm=TRUE)))
    message(paste("action4 TRUE count:", sum(action4, na.rm=TRUE)))
    message(paste("action5 TRUE count:", sum(action5, na.rm=TRUE)))
    
    # Calculate action score by summing logical vectors directly
    df$action_score <- rowSums(
      cbind(action1, action2, action3, action4, action5),
      na.rm = TRUE
    )
    
    # Debug: Check distribution of action_score
    action_score_table <- table(df$action_score)
    message("action_score distribution:")
    print(action_score_table)
